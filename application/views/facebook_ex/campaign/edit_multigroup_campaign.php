	...
	<div class="col-xs-12 col-md-7 padding-10">
		<div class="box box-primary">
			<div class="box-header ui-sortable-handle  text-center" style="cursor: move;margin-bottom: 0px;">
				...
				<h3 class="box-title"><?php echo $this->lang->line("edit multi-group campaign") ?></h3>
				<!-- tools box -->
				...
				<form action="#" enctype="multipart/form-data" id="inbox_campaign_form" method="post">
					...
					<div class="form-group">
						<label style="width:100%">
							<?php echo $this->lang->line("choose groups") ?> *
							 <a href="#" data-placement="top" data-toggle="popover" data-trigger="focus" title="<?php echo $this->lang->line("choose groups") ?>" data-content="<?php echo $this->lang->line("Choose one or more groups to create campaign. This message will send to all your active leads of groups you choose now. You can use 'Do not send message to these leads' field below to unlist any list only from this campaign. To unlist any specific lead permanently, please go to 'Import Lead > Lead List' and unsubscribe the lead, he/she will not recieve any other message until he/she is subscribed again. System will filter multiple instances of same lead for same campaign, means one user will not recieve same campaign message multiple times.") ?>"><i class='fa fa-info-circle'></i> </a>
							 <span class='label label-info pull-right' style="padding:3px 8px;font-size:13px;-webkit-border-radius: 10px;-moz-border-radius: 10px;border-radius: 10px;">Total Lead Selected : <span id="thread_count"><?php echo $xthread_count ?></span></span>
						</label>
...
<script>
	...
	$j("document").ready(function(){
        ...
        $(document.body).on('click','.ms-parent input[type=checkbox]',function(){   
        	...
        	var anywaywewillsubmitallleadsforsendingmessage = "<?php echo $anywaywewillsubmitallleadsforsendingmessage;?>";
        	var sorryyourmonthlylimittosendmessageisxceede = "<?php echo $sorryyourmonthlylimittosendmessageisxceede;?>";
    		...
    		$.ajax({
		       ...
		       { 
		       	 if(count > 300)
		       	 {
		       	 	...
		       	 	var alert_message = thelistseemslargeehighlyrecommendtoplityourcampaign+" <a href="+custom_campaign_link+">"+gohere+"</a>. "+anywaywewillsubmitallleadsforsendingmessage;
		       	 ...
		       	 {
		       	 	alert(sorryyourmonthlylimittosendmessageisxceede);
	    ...
	    $(document.body).on('click','#submit_post',function(){ 
        	...
        	var inbox_to_pages = $("#inbox_to_pages").val();
        	var pleaseselectgroupstocreateinboxcampaign = "<?php echo $pleaseselectgroupstocreateinboxcampaign; ?>";
        	if(inbox_to_pages==null)
        	{
        		alert(pleaseselectgroupstocreateinboxcampaign);
        		return;
        	}
        	...
        	$("#response_modal_content").removeClass("alert-danger");
         	...
         	$("#response_modal_content").html(success_message);
		      ...
		      $.ajax({
			       type:'POST' ,
			       url: base_url+"facebook_ex_campaign/edit_multigroup_campaign_action",
			       data: queryString,
			       cache: false,
			       contentType: false,
			       processData: false,
